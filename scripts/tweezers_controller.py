...
class PhSensorController:
    def __init__(self):
        rospy.init_node('tweezers_controller', anonymous=True)
        ...
        # 服务
        self.srv_measure = rospy.Service("/tweezers/draw", TweezersDraw, self.cb_measure)
        rospy.loginfo("[ph_sensor_controller] ready. Waiting for /tweezers/draw calls...")
    ...
    def cb_measure(self, req):
        try:
            ...
            # == 整套流程 ==
            if not self.pick_tweezers():
                return TweezersDrawResponse(False, "pick failed")

            if not self.draw_filament():
                return TweezersDrawResponse(False, "draw failed")

            return TweezersDrawResponse(True,'draw filament successfully')

        except Exception as e:
            rospy.logerr(f"draw filament failed: {e}")
            return TweezersDrawResponse(False, str(e))
    ...
    # ---------------- 高阶流程 ----------------
    def pick_tweezers(self):
        """1) 前往抓取 pH sensor：先到架上方，再下降抓取，再抬起"""

        rospy.loginfo("Picking tweezers...")
        # a. 架上方安全高度
        self.open_gripper()
        ...
        # b. 下降到传感器中心附近
        z_mm3 = 158.6
        self.go_to_arm_pose([x_mm2, y_mm2, z_mm3, roll2, pitch2, yaw2])
        rospy.sleep(2)
        self.close_gripper(distance=18)
        rospy.sleep(0.1)
        self.close_gripper(distance=13)
        rospy.sleep(0.1)
        # c. 抬回安全高度
        ...
        self.close_gripper(distance=8)
        rospy.sleep(0.1)
        self.close_gripper(distance=3)
        rospy.sleep(0.1)
        self.close_gripper(distance=13)
        rospy.sleep(0.1)
        return True

    def draw_filament(self):
        """2) 前往目标位置并下沉；3) 等待1s并读取 pH；4) 上升"""

        rospy.loginfo("Go to draw filament...")
        x_mm = -126.4
        y_mm = 228
        z_mm = 389
        roll = 176.3
        ...
        # b. 下沉
        rospy.loginfo("down and draw up...")
        z_mm3 = 157
        self.go_to_arm_pose([x_mm2, y_mm2, z_mm3, roll, pitch, yaw])
        rospy.sleep(1)
